...
subscribeApp.controller("CampusAndBuildingCtrl",function($scope,subscribeService){
    ...
    if(RR) {
        ...
        $scope.RR_change = function(bc){
            ...
            if(bc){
                ...
                $scope.subscribeDate1 = subscribeService.getDate1();
    ...
    $scope.checkDate = function (arg) {
        ...
        if(arg==0){//点击第二天
            $scope.subscribeDate1 = subscribeService.getDate2(0);
        }else{
            $scope.subscribeDate1 = subscribeService.getDate2(arg);
        }
    }
    $scope.checkDateEnd = function (arg) {
        subscribeService.checkDateEnd(arg);
    }
...
subscribeApp.factory("subscribeService",function () {
    ...
    factory.cookie_subscribeDate1 = g.toJson($.cookie("subscribeDate1"));
    ...
    factory.getDate = function(){
        ...
        var subscribeDate = new Date(curDate.getTime()+ 24*60*60*1000);
        ...
        y_day = y_day ? y_day : days.reservationTime;
    ...
    factory.getDate1 = function(){
       ...
       // var subscribeDate = new Date();
        ...
        var luch=days.reservationLength;

        //y_day = y_day ? y_day : luch;
        if(y_day=y_day){
            if(y_day>luch){
                y_day=y_day;
            }else{
        ...
        //y_day > luch ? y_day : luch;

        var S_date = subscribeDate.getDate();

        for(var d = 1 ; d <= y_day ; d++){
            //var D_Date =  subscribeDate.setDate( S_date + d);

            //var date = new Date(D_Date);
            var date=new Date(curDate.getTime()+ d*24*60*60*1000);
            var dates = {};
            dates.Weekday = weekday[parseInt(S_Day+d-1) % 7];
            var Month = date.getMonth()+1  ;
            Month = Month.toString().length == 2 ? Month : "0"+ Month;
            var Day = date.getDate().toString();
            Day = Day.length == 2 ? Day : "0" + Day;
            dates.date = Month + "-" + Day;
            if(d == 1){
                factory.dayEnd = dates.date;
            }
            if(typeof factory.radioType == "undefined" || factory.radioType == 1 ){

                if( factory.cookie_subscribeDate1 &&  factory.cookie_subscribeDate1.reservationDay){
                    dates.active = dates.date ==  factory.cookie_subscribeDate1.reservationDay ? "active" : "";
                }else{
                    dates.active = d == 1 ? "active" : "";
                }
    ...
    factory.getDate2 = function(args){
        ...
        var argss=1;
        if(args==0){
            var subscribeDate = new Date(curDate.getTime()+ 24*60*60*1000);//后两天
        }else{
            var subscribeDate = new Date(curDate.getTime()+ (args+1)*24*60*60*1000);//后一天
            argss==2;
        }
       ...
       // y_day = y_day ? y_day : luch;
        ...
        for(var d = 1 ; d < y_day ; d++){
            ...
            //var date = new Date(D_Date);
            var date=new Date(subscribeDate.getTime()+ (d-1)*24*60*60*1000);
            var dates = {};
            dates.Weekday = weekday[parseInt(S_Day+d-1) % 7];
            var Month = date.getMonth()+1  ;
            Month = Month.toString().length == 2 ? Month : "0"+ Month;
    ...
    factory.checkDate = function (arg) {
        ...
        if(typeof factory.radioType == "undefined" || factory.radioType == 1 ) {
            ...
            factory.dayStart = day_span.eq(1)[0].innerHTML;
    ...
    factory.checkDateEnd = function (arg) {
        ...
        if(typeof factory.radioType == "undefined" || factory.radioType == 1 ) {
            var day_span = day_children.find("span");
            factory.dayEnd = day_span.eq(1)[0].innerHTML;
            day_children.addClass("active").siblings().removeClass("active");
    ...
    factory.ChooseSeat = function (arg) {
        if(arg != null && ( arg == "optionalSeatDay" || arg == "randomSeatDay")){
            var bean = {};
            var Day_year = new Date().getFullYear();


            bean.reservationBeginTime = Day_year + "-" + factory.dayStart + " "+ $(".date #start")[0].innerHTML;
            bean.reservationEndTime = Day_year + "-" + factory.dayEnd + " "+ $(".date #end")[0].innerHTML;
            bean.reservationDate = Day_year + "-" + factory.dayStart ;
            bean.reservationDay =  factory.dayEnd ;
            ...
            var res= Day_year + "-" + factory.dayStart + " "+ $(".date #start")[0].innerHTML;
            var endres=Day_year + "-" + factory.dayEnd + " "+ $(".date #end")[0].innerHTML;


            var  startDateTime=res.replace(/-/g, "/");
            var endDateTime=endres.replace(/-/g, "/");

            var date1=new Date(endDateTime);
            var date2=new Date(startDateTime);
            var starts=date1.getTime();
            var startss=date2.getTime();
            if(starts<startss){
                factory.alertError("结束时间不能小于开始时间！");
                return false;
            }
