...
app.controller("subscribeListCtrl",function($scope,appService,$interval){
    ...
    $scope.subscribeDay=function(){
        var userinfos=g.userInfo();
        var luck=userinfos.reservationLengthRule;
        if(luck==2){
            window.location = "subscribeDay.html";
        }else{
    ...
    $scope.checkIn = function(){
        var sl = this.sl;
        var resercationBeginTimes = sl.sreservationBeginTime;
        var myDate = new Date();
        var resultTimes=resercationBeginTimes.replace("-" , "/").replace("-" , "/");
        var startTimes=new Date(resultTimes);
        if(startTimes!=""&&myDate!=""&&myDate<startTimes)
        {
            mui.toast("预约时间未到，不能签到！");
            return false;
        }else{
            wx.scanQRCode({
                needResult: 1,
                ...
                success: function (res) {
                    ...
                    var reApi = Api.checkInSeatDay(rs);
                    if(reApi.success && reApi.message != 0){
                        $scope.$apply(function () {
                            var subscribeListTwo = appService.selectReservationByUser();
                            if(subscribeListTwo) {
    ...
    $scope.contract = function () {
        ...
        var userinfos=g.userInfo();
        var rule=userinfos.reservationLengthRule;
        if (rule != 2) {
            window.location = "contract.html";
        } else {
            var resercationEsdTimes = sl.sreservationEndTime;
            var startTimes = resercationEsdTimes.split(" ");
            var startTimes1 = startTimes[0].split("-");
            var startDates=parseInt(startTimes1[0])+"-"+parseInt(startTimes1[1])+"-"+parseInt(startTimes1[2]);
            var myDate = new Date();
            var result=parseInt(myDate.getFullYear())+"-"+parseInt((myDate.getMonth()+1))+"-"+parseInt(myDate.getDate());
            if(result!=""&&startDates!=""&&startDates!=result)
            {
                mui.toast("预约时长的最后一天才能续约！");
                return false;
            }
            window.location = "contractDay.html";
        }
    ...
    $scope.checkSeat = function () {
        ...
        var luck=userinfos.reservationLengthRule;
        if(luck==2){
            window.location = "checkSeatDay.html";
        }else{
...
app.factory("appService",function () {
    ...
    factory.yxsApply = function(){
        ...
        var list = Api.selectReservationByUserDay();
        if(!list.success){
            $(".app-null").show();
...
app.directive("zwlzxsindex",function(){
    var userinfos=g.userInfo();
    var reservationRule=userinfos.reservationLengthRule;
    if(reservationRule==2){
        return {
            templateUrl : "template/zxs_indexDay.html"
        };
    }else{
        return {
